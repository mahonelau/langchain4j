...
public class KmsFileLoader {
     ...
     */
    public static KmsDocument loadDocument(String text, String fileId, String docId, String[] topicCode, String title,Integer entType,int releaseFlag) {
        //entType 1:doc,2:file
        if (!Objects.equals(entType, KmsDocBase.ENT_TYPE_FILE) || fileId == null || docId == null) {
            throw illegalArgument("entType:%s or entId:%s is error", entType, fileId + docId);
        }

        return load(from(text,fileId,docId,topicCode,title,entType,releaseFlag));
    }
    /**
     * Loads a document from the specified file.
     * Returned document contains all the textual information from the file.
    ...
    public static KmsDocument loadDocument(String text, KmsDocBase kmsDocBase) {
        if (!Objects.equals(kmsDocBase.getEntType(), KmsDocBase.ENT_TYPE_FILE) || kmsDocBase.getDocId() == null || kmsDocBase.getFileId() == null) {
            throw illegalArgument("entType:%s or docId:%s  or fileId:%s is error", kmsDocBase.getEntType(),  kmsDocBase.getDocId(),  kmsDocBase.getFileId());
        }

        return load(KmsFileSource.from(text,kmsDocBase));
    }
